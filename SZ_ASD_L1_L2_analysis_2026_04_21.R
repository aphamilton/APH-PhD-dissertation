...
# Set a permanent output directory (and adjust existing .csv sections)
# Create a section at the end of the analyses that saves the plots (put stop() before)
# Further analysis of bilingualism variables (for CPA)
...
# New version of previous function that accepts up to four cols
# Used for AoA data
# Set en_col2_name and fr_col2_name to NULL if they're not needed
# This can only be used after the "is_l1_en" column is created
add_merged_col_many <- function(my_df,
                                en_col1_name, en_col2_name,
                                fr_col1_name, fr_col2_name,
                                new_col_name)
{
  my_df$new_col <- NA
  colnames(my_df)[which(colnames(my_df)=="new_col")] <- new_col_name
  for (my_row in 1:nrow(my_df))
  {
    if (my_df[my_row, "is_l1_en"] == 1)
    {
      if ((!is.na(my_df[my_row, en_col1_name])) &&
          (my_df[my_row, en_col1_name] != ""))
        my_df[my_row, new_col_name] <- my_df[my_row, en_col1_name]
      if ((!is.null(en_col2_name)) && 
          (!is.na(my_df[my_row, en_col2_name])) &&
          (my_df[my_row, en_col2_name] != ""))
        my_df[my_row, new_col_name] <- my_df[my_row, en_col2_name]
    }
    else
    {
      if ((!is.na(my_df[my_row, fr_col1_name])) &&
          (my_df[my_row, fr_col1_name] != ""))
        my_df[my_row, new_col_name] <- my_df[my_row, fr_col1_name]
      if ((!is.null(fr_col2_name)) && 
          (!is.na(my_df[my_row, fr_col2_name])) &&
          (my_df[my_row, fr_col2_name] != ""))
        my_df[my_row, new_col_name] <- my_df[my_row, fr_col2_name]
    }
  }
  return(my_df)
}
...
my_data <- within(my_data, source[random_id == 33534377 & 
                                    source == "sona_cogsci"] <- "sona_psych")
my_data <- within(my_data, source[random_id == 98996051 & 
                                    source == "sona_psych"]  <- "sona_cogsci")
...
my_data <- subset(my_data, my_data$eligibility_age == "Yes / Oui" | 
                            # Allowing empty age eligibility response for Prolific participants only
                              my_data$eligibility_age == "" & 
                                (my_data$source == "prolific_l1e" | 
...
my_data <- subset(my_data, my_data$eligibility_language == "Yes / Oui" |
                           # 2023 data - Prolific
                           (my_data$eligibility_language == "" & 
                              (my_data$source == "prolific_l1e" | 
                                 my_data$source == "prolific_l1f")) |
                           # Newer data - bilinguals
                           (my_data$eligibility_english == "Yes / Oui" & 
                              my_data$eligibility_french == "Yes / Oui") |
                           # Newer data - monolinguals
                           (my_data$eligibility_english == "Yes / Oui" & 
                              my_data$eligibility_french == "No / Non" & 
                              eligibility_other_lan == "No / Non")
                 )

my_data <- subset(my_data, my_data$first_language == "English / Anglais" | 
                              my_data$first_language == "French / Français")
my_data <- subset(my_data, my_data$l1 == "English / Anglais" | 
                              my_data$l1 == "French / Français")
...
my_data <- subset(my_data, my_data$source == "sona_cogsci" | 
...
qualtrics_test_runs <- c("R_ex2UqHGk3N4rr4l", "R_0p64Pt4Pn1xe5fb",
                         "R_2YbRN1GFsE1FhdG", "R_1k17QY4T3sbgvwZ",
                         "R_ywfTKwA3Qcc1lXH", "R_0p64Pt4Pn1xe5fb",
                         "R_65tMJaBcWfyAVQf", "R_3CrHVKG4Db5xIgB",
                         "R_7aP80Int6z55TNk", "R_5kjMPUh4nf7uvIz")
...
my_data_prev <- subset(my_data_prev, my_data_prev$eligibility_age == "Yes / Oui" | 
                                     my_data_prev$eligibility_age == "" & 
                                        (my_data_prev$source == "prolific_l1e" | 
                                         my_data_prev$source == "prolific_l1f"))
...
my_data_prev <- subset(my_data_prev, my_data_prev$source == "sona_cogsci" | 
                                        my_data_prev$source == "sona_psych" | 
                                        my_data_prev$source == "prolific_l1e" | 
                                        my_data_prev$source == "prolific_l1f")
...
# my_data_prev_exp <- 
#   my_data_prev[,c("response_id",
#                   "dlq_e_3a","dlq_e_3b",
#                   "dlq_e_8a","dlq_e_8b","dlq_e_8c","dlq_e_8d",
#                   "dlq_f_3a","dlq_f_3b",
#                   "dlq_f_8a","dlq_f_8b","dlq_f_8c","dlq_f_8d")]
# setwd("C:/Users/Arthur/OneDrive/Documents/R/PhD_Thesis/SZ_ASD_L1_L2_analysis/Output")
# write.csv(my_data_prev_exp, "AH_Thesis_DLQ_3_8.csv")
# setwd("C:/Users/Arthur/OneDrive/Documents/R/PhD_Thesis/SZ_ASD_L1_L2_analysis/Data")

# Reduce to only the cols needed for merging to main df
my_data_prev <- my_data_prev[,c("random_id",
                                "dlq_e_1","dlq_e_11",
                                "dlq_f_1","dlq_f_11")]
...
dlq_3_data     <- read.csv(file = "DLQ_3_Coded.csv",
                           encoding="UTF-8")
dlq_4_5_6_data <- read.csv(file = "DLQ_4_5_6_Coded.csv",
                           encoding="UTF-8")
dlq_7_data     <- read.csv(file = "DLQ_7_Coded.csv",
                           encoding="UTF-8")
dlq_8_data     <- read.csv(file = "DLQ_8_Coded_Revised.csv",
                           encoding="UTF-8")
...
rm(dlq_3_data, dlq_4_5_6_data, dlq_7_data, dlq_8_data, my_col)
...
# Now merge AoA data (new and old versions, En and Fr)
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_1", "dlq_e_a4_1",
                               "dlq_f_a4r_1", "dlq_f_a4_1",
                               "en_aoa_speak")
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_2", "dlq_e_a4_2",
                               "dlq_f_a4r_2", "dlq_f_a4_2",
                               "en_aoa_speak_flu")
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_3", NULL,
                               "dlq_f_a4r_3", "dlq_f_a4_3",
                               "en_aoa_read")
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_4", NULL,
                               "dlq_f_a4r_4", "dlq_f_a4_4",
                               "en_aoa_read_flu")
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_6", NULL,
                               "dlq_f_a4r_5", NULL,
...
my_data <- add_merged_col_many(my_data,
                               ...
                               "fr_aoa_speak_flu")
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_8", "dlq_e_a4_3",
                               "dlq_f_a4r_7", NULL,
                               "fr_aoa_read")
my_data <- add_merged_col_many(my_data,
                               "dlq_e_a4r_9", "dlq_e_a4_4",
                               "dlq_f_a4r_8", NULL,
                               "fr_aoa_read_flu")

# Convert AoA cols to numeric
my_data <- my_data %>%
  mutate(across(c(en_aoa_speak, en_aoa_speak_flu,
                  en_aoa_read,  en_aoa_read_flu,
                  fr_aoa_speak, fr_aoa_speak_flu,
                  fr_aoa_read,  fr_aoa_read_flu),
                as.numeric))

# AoA cols - writing them out to keep straight
# Began speaking English         -> dlq_e_a4r_1 OR dlq_f_a4r_1 OR dlq_e_a4_1 OR dlq_f_a4_1
# Became fluent speaking English -> dlq_e_a4r_2 OR dlq_f_a4r_2 OR dlq_e_a4_2 OR dlq_f_a4_2
# Began reading English          -> dlq_e_a4r_3 OR dlq_f_a4r_3 OR dlq_f_a4_3
# Became fluent reading English  -> dlq_e_a4r_4 OR dlq_f_a4r_4 OR dlq_f_a4_4
# Began speaking French          -> dlq_e_a4r_6 OR dlq_f_a4r_5
# Became fluent speaking French  -> dlq_e_a4r_7 OR dlq_f_a4r_6
# Began reading French           -> dlq_e_a4r_8 OR dlq_f_a4r_7 OR dlq_e_a4_3
# Became fluent reading French   -> dlq_e_a4r_9 OR dlq_f_a4r_8 OR dlq_e_a4_4

# Calculate num_langs for more recent participants
# NOTE: If we start using the monolingual participants, this has to be adjusted
for (my_row in 1:nrow(my_data))
{
  # Excluding 2023 participants from this - they had a free-responses format
  if (substr(my_data[my_row, "start_date"], 1, 4) != "2023")
  {
    # If they completed DLQ in English and did not skip question
    if (my_data[my_row, "dlq_e_3r"] != "" && 
        !is.na(my_data[my_row, "dlq_e_3r"]))
    {
      my_num_langs <- 2 + 
        (str_count(my_data[["dlq_e_3r"]][my_row],
                   fixed("Other language if applicable (please specify):")))
      my_data[my_row, "num_langs"] <- my_num_langs
    }
    
    # If they completed DLQ in French and did not skip question
    if (my_data[my_row, "dlq_f_3r"] != "" && 
        !is.na(my_data[my_row, "dlq_f_3r"]))
    {
      my_num_langs <- 2 +  
        (str_count(my_data[["dlq_f_3r"]][my_row],
                   fixed("Autre langue le cas échéant (veuillez préciser):")))
      my_data[my_row, "num_langs"] <- my_num_langs
    }
  }
}
rm(my_row, my_num_langs)
...
rm(cur_partic, cur_id, corresp_row)
...
my_data[my_data$age == "18 years old", "age"] <- "18"
my_data[my_data$age == "18 ans", "age"] <- "18"
my_data[my_data$age == "`18", "age"] <- "18"
my_data[my_data$age == " 19", "age"] <- "19"
my_data[my_data$age == "dix neuf", "age"] <- "19"
my_data[my_data$age == "19 ans", "age"] <- "19"
my_data[my_data$age == "20 years old", "age"] <- "20"
my_data[my_data$age == "20 ans", "age"] <- "20"
my_data[my_data$age == "26 ans", "age"] <- "26"
# # Code to identify values of age that cannot convert to numeric - run again if new participants
# bad_vals <- my_data$age[is.na(as.numeric(my_data$age))]
# unique(bad_vals)
# rm(bad_vals)

# Convert all to numeric
# Suppressing the warning that NAs were introduced by coercion (this is desired)
my_data$age <- suppressWarnings(as.numeric(my_data$age))
# NA for participants who presumably did not take this seriously
my_data$age[!is.na(my_data$age) & my_data$age >= 80] <- NA
my_data$age[!is.na(my_data$age) & my_data$age == 0] <- NA

# Gender

my_data <- add_merged_col(my_data, "dlq_e_2", "dlq_f_2", "gender")
my_data[(my_data$gender == "Female" | 
           my_data$gender == "Femme"), 
        "gender"] <- "0"
my_data[(my_data$gender == "Male" | 
           my_data$gender == "Homme"), 
        "gender"] <- "1"
my_data[(my_data$gender == "Other (please specify)" | 
           my_data$gender == "Autre (veuillez préciser)"), 
        "gender"] <- "2"
my_data$gender <- suppressWarnings(as.numeric(my_data$gender))

# Diagnoses

my_data <- add_merged_col(my_data, "dlq_e_11r", "dlq_f_11r", "diagnoses")
my_data <- add_merged_col(my_data, "dlq_e_12r", "dlq_f_12r", "diagnoses_parents")
my_data <- add_merged_col(my_data, "dlq_e_13r", "dlq_f_13r", "diagnoses_siblings")

# There are also some additional options for participants, need to check the questions
# "Other learning disability|Une autre trouble d’apprentissage"
# "Vision problem|mes de vision"
# "Hearing impairment|mes d’audition"

# Chose the French strings to not include accents, since had some issues with accents before
my_data$diag_asd  <- grepl("Autism spectrum disorder|Trouble du spectre",
                           my_data$diagnoses)
my_data$diag_adhd <- grepl("ADHD|TDAH",
                           my_data$diagnoses)
my_data$diag_sli  <- grepl("Specific Language Impairment|cifique du langage",
                           my_data$diagnoses)
my_data$diag_dysl <- grepl("Dyslexia|Dyslexie",
                           my_data$diagnoses)
my_data$diag_ld   <- grepl("Other learning disability|Une autre trouble d’apprentissage",
                           my_data$diagnoses)
my_data$diag_epil <- grepl("Epilepsy|pilepsie",
                           my_data$diagnoses)
my_data$diag_sz   <- grepl("Schizophrenia|trouble psychotique",
                           my_data$diagnoses)
my_data$diag_mood <- grepl("Mood disorder|pression ou trouble bipolaire",
                           my_data$diagnoses)
my_data$diag_anx  <- grepl("Anxiety disorder|Trouble anxieux",
                           my_data$diagnoses)
my_data$diag_vis  <- grepl("Vision problem|mes de vision",
                           my_data$diagnoses)
my_data$diag_hear <- grepl("Hearing impairment|mes d’audition",
                           my_data$diagnoses)

# And now for relatives' diagnoses
my_data$diag_rel_asd  <-
  grepl("Autism spectrum disorder|Trouble du spectre",
        my_data$diagnoses_siblings) |
  grepl("Autism spectrum disorder|Trouble du spectre",
        my_data$diagnoses_parents)
my_data$diag_rel_adhd <-
  grepl("ADHD|TDAH",
        my_data$diagnoses_siblings) |
  grepl("ADHD|TDAH",
        my_data$diagnoses_parents)
my_data$diag_rel_sli  <-
  grepl("Specific Language Impairment|cifique du langage",
        my_data$diagnoses_siblings) |
  grepl("Specific Language Impairment|cifique du langage",
        my_data$diagnoses_parents)
my_data$diag_rel_dysl <-
  grepl("Dyslexia|Dyslexie",
        my_data$diagnoses_siblings) |
  grepl("Dyslexia|Dyslexie",
        my_data$diagnoses_parents)
my_data$diag_rel_ld   <-
  grepl("Other learning disability|Une autre trouble d’apprentissage",
        my_data$diagnoses_siblings) |
  grepl("Other learning disability|Une autre trouble d’apprentissage",
        my_data$diagnoses_parents)
my_data$diag_rel_epil <-
  grepl("Epilepsy|pilepsie",
        my_data$diagnoses_siblings) |
  grepl("Epilepsy|pilepsie",
        my_data$diagnoses_parents)
my_data$diag_rel_sz   <-
  grepl("Schizophrenia|trouble psychotique",
        my_data$diagnoses_siblings) |
  grepl("Schizophrenia|trouble psychotique",
        my_data$diagnoses_parents)
my_data$diag_rel_mood <-
  grepl("Mood disorder|pression ou trouble bipolaire",
        my_data$diagnoses_siblings) |
  grepl("Mood disorder|pression ou trouble bipolaire",
        my_data$diagnoses_parents)
my_data$diag_rel_anx  <-
  grepl("Anxiety disorder|Trouble anxieux",
        my_data$diagnoses_siblings) |
  grepl("Anxiety disorder|Trouble anxieux",
        my_data$diagnoses_parents)
my_data$diag_rel_vis  <-
  grepl("Vision problem|mes de vision",
        my_data$diagnoses_siblings) |
  grepl("Vision problem|mes de vision",
        my_data$diagnoses_parents)
my_data$diag_rel_hear <-
  grepl("Hearing impairment|mes d’audition",
        my_data$diagnoses_siblings) |
  grepl("Hearing impairment|mes d’audition",
        my_data$diagnoses_parents)

# Need to add the additional 3 diagnoses here
# For participants who checked all diagnoses, set to NA
rows_all_diags <-
  which(my_data$diag_asd  == TRUE & my_data$diag_adhd == TRUE &
        my_data$diag_sli  == TRUE & my_data$diag_dysl == TRUE &
        my_data$diag_ld   == TRUE & my_data$diag_epil == TRUE & 
        my_data$diag_sz   == TRUE & my_data$diag_mood == TRUE & 
        my_data$diag_anx  == TRUE & my_data$diag_vis  == TRUE &
        my_data$diag_hear == TRUE)
my_data[rows_all_diags,
        c("diag_asd", "diag_adhd", "diag_sli", "diag_dysl",
          "diag_ld",  "diag_epil", "diag_sz",  "diag_mood",
          "diag_anx", "diag_vis",  "diag_hear")] <- NA
# And same for participants who checked all family diagnoses
rows_all_rel_diags <-
  which(my_data$diag_rel_asd  == TRUE & my_data$diag_rel_adhd == TRUE &
        my_data$diag_rel_sli  == TRUE & my_data$diag_rel_dysl == TRUE &
        my_data$diag_rel_ld   == TRUE & my_data$diag_rel_epil == TRUE & 
        my_data$diag_rel_sz   == TRUE & my_data$diag_rel_mood == TRUE & 
        my_data$diag_rel_anx  == TRUE & my_data$diag_rel_vis  == TRUE &
        my_data$diag_rel_hear == TRUE)
my_data[rows_all_rel_diags,
        c("diag_rel_asd", "diag_rel_adhd",
          "diag_rel_sli", "diag_rel_dysl",
          "diag_rel_ld",  "diag_rel_epil",
          "diag_rel_sz",  "diag_rel_mood",
          "diag_rel_anx", "diag_rel_vis",
          "diag_rel_hear")] <- NA
# Clean up environment
rm(rows_all_diags, rows_all_rel_diags)
...
my_data <- my_data %>%
  mutate(across(c(en_exposure, fr_exposure,
                  en_reading, fr_reading,
                  en_speaking, fr_speaking),
                ~ replace(., . > 100, NA)))

# And do same for values over 10 (for proficiency)
my_data <- my_data %>%
  mutate(across(c(prof_e_speak,   prof_f_speak,
                  prof_e_underst, prof_f_underst,
                  prof_e_write,   prof_f_write,
                  prof_e_read,    prof_f_read),
                ~ replace(., . > 10, NA)))

# Remove values over 80 years
my_data <- my_data %>%
  mutate(across(c(en_country_yrs, fr_country_yrs,
                  en_family_yrs, fr_family_yrs,
                  en_school_work_yrs, fr_school_work_yrs,
                  en_aoa_speak, fr_aoa_speak,
                  en_aoa_speak_flu, fr_aoa_speak_flu,
                  en_aoa_read, fr_aoa_read,
                  en_aoa_read_flu, fr_aoa_read_flu),
                ~ replace(., . > 80, NA)))

# Remove age of fluency if below 5 years (implausible)
my_data <- my_data %>%
  mutate(across(c(en_aoa_speak_flu, fr_aoa_speak_flu,
                  en_aoa_read_flu,  fr_aoa_read_flu),
                ~ replace(., . < 5, NA)))

# Set values to NA where age they started learning is greater than age of fluency
my_data <- my_data %>%
  mutate(
    en_aoa_speak     = if_else(en_aoa_speak > en_aoa_speak_flu,
                               NA, en_aoa_speak),
    en_aoa_speak_flu = if_else(en_aoa_speak > en_aoa_speak_flu,
                               NA, en_aoa_speak_flu),
    en_aoa_read      = if_else(en_aoa_read  > en_aoa_read_flu,
                               NA, en_aoa_read),
    en_aoa_read_flu  = if_else(en_aoa_read  > en_aoa_read_flu,
                               NA, en_aoa_read_flu),
    fr_aoa_speak     = if_else(fr_aoa_speak > fr_aoa_speak_flu,
                               NA, fr_aoa_speak),
    fr_aoa_speak_flu = if_else(fr_aoa_speak > fr_aoa_speak_flu,
                               NA, fr_aoa_speak_flu),
    fr_aoa_read      = if_else(fr_aoa_read  > fr_aoa_read_flu,
                               NA, fr_aoa_read),
    fr_aoa_read_flu  = if_else(fr_aoa_read  > fr_aoa_read_flu,
                               NA, fr_aoa_read_flu)
  )

# Remove values that are greater than their age (with a 1-year extra margin)
my_data <- my_data %>%
  mutate(across(
    c(en_country_yrs, fr_country_yrs,
      en_family_yrs, fr_family_yrs,
      en_school_work_yrs, fr_school_work_yrs,
      en_aoa_speak, fr_aoa_speak,
      en_aoa_speak_flu, fr_aoa_speak_flu,
...
rm(fhat_data, non_na_groups_dsct, non_na_groups_sst, non_na_groups_fhat,
   ...
   non_empty_pavlovia_files, empty_pavlovia_files, i)


# ====== QC checks on question blocks ===== ----

# Create df of block cols ----
...
add_block_cols <- function(block_num, block_name,
                           first_col_name, last_col_name,
                           num_questions)
{
  ...
  block_col_nums[block_num,"num_questions"] <<- num_questions
}

# Add the column numbers for the blocks
add_block_cols(1, "lt_e",   "lt_e_1",   "lt_e_30",   30)
add_block_cols(2, "lt_f",   "lt_f_1",   "lt_f_30",   30)
add_block_cols(3, "ca_c_e1","ca_c_e1_1","ca_c_e1_16",16)
add_block_cols(4, "ca_c_f2","ca_c_f2_1","ca_c_f2_16",16)
add_block_cols(5, "ca_c_e2","ca_c_e2_1","ca_c_e2_16",16)
add_block_cols(6, "ca_c_f1","ca_c_f1_1","ca_c_f1_16",16)
add_block_cols(7, "sm_e1",  "sm_e1_1",  "sm_e1_16",  16)
add_block_cols(8, "sm_f2",  "sm_f2_1",  "sm_f2_16",  16)
add_block_cols(9, "sm_e2",  "sm_e2_1",  "sm_e2_16",  16)
add_block_cols(10,"sm_f1",  "sm_f1_1",  "sm_f1_16",  16)
add_block_cols(11,"p_e1",   "p_e1_1",   "p_e1_18",   18)
add_block_cols(12,"p_f2",   "p_f2_1",   "p_f2_18",   18)
add_block_cols(13,"p_e2",   "p_e2_1",   "p_e2_18",   18)
add_block_cols(14,"p_f1",   "p_f1_1",   "p_f1_18",   18)
add_block_cols(15,"mss_b_e","mss_b_e_1","mss_b_e_38",38)
add_block_cols(16,"asq_e",  "asq_e_1",  "asq_e_50",  50)
add_block_cols(17,"mss_b_f","mss_b_f_1","mss_b_f_38",38)
add_block_cols(18,"asq_f",  "asq_f_1",  "asq_f_50",  50)


# Checking for missingness and sameness by block ----

# Listing blocks where repeated answers are possible
blocks_can_repeat <- c("lt_e","lt_f",
                       "sm_e1","sm_e2","sm_f1","sm_f2",
                       "mss_b_e","mss_b_f",
                       "asq_e","asq_f")

for (my_row in 1:nrow(my_data))
{
  # Calculate the number of blank responses per block per participant
  for (my_block in 1:nrow(block_col_nums))
  {
    first_col <- block_col_nums[my_block, "first_col"]
    last_col  <- block_col_nums[my_block, "last_col"]
    subset_mat <- as.matrix(my_data[my_row, first_col:last_col])
    number_blank <- sum(is.na(subset_mat)) + sum(subset_mat == "", na.rm = TRUE)
    new_my_data_col <- paste0("blanks_",
                              block_col_nums[my_block, "block"])
    my_data[my_row, new_my_data_col] <- number_blank
  }
  
  # Check for repeated responses where applicable
  for (my_block in blocks_can_repeat)
  {
    first_col <- block_col_nums[block_col_nums$block == my_block,
                                "first_col"]
    last_col  <- block_col_nums[block_col_nums$block == my_block,
                                "last_col"]
    subset_vec <- unlist(my_data[my_row, first_col:last_col])
    subset_vec <- subset_vec[!is.na(subset_vec) & subset_vec != ""]
    new_my_data_col <- paste0("same_", my_block)
    if (length(table(subset_vec)) == 0)
      my_data[my_row, new_my_data_col] <- NA
    else
      my_data[my_row, new_my_data_col] <- max(table(subset_vec))
  }
}
rm(first_col, last_col, subset_mat, subset_vec,
   number_blank, new_my_data_col)


# ====== Score tasks and questionnaires ====== ----
...
for (my_row in 1:nrow(my_data))
{
  ...
  my_data[my_row, "prof_e_total"] <- sum(my_data[my_row, c("prof_e_speak","prof_e_underst",
                                                           "prof_e_write","prof_e_read")])
  my_data[my_row, "prof_f_total"] <- sum(my_data[my_row, c("prof_f_speak","prof_f_underst",
                                                           "prof_f_write","prof_f_read")])
}
rm(my_row)
...
# ====== Further QC / removing participants ====== ----
...
# Filter by bilingual status and age ----

# Remove monolingual group
...
# Initialize the df for tracking exclusions ----

# Create the empty overarching list for results to be added to
my_results <- list()

# Create first df within list to track participant exclusions
my_results[["tracking_exclusions"]] <- 
  as.data.frame(matrix(NA, nrow = 5, ncol = 2))
colnames(my_results[["tracking_exclusions"]]) <- c("point", "N")
my_results[["tracking_exclusions"]]$point <- c("Initial",
                                               "After_MissingDataFilter",
                                               "After_QC",
                                               "After_DxFilter",
                                               "After_RelDxFilter")
# Add number of participants before QC
my_results[["tracking_exclusions"]][1,"N"] <- nrow(my_data)


# Remove participants with missing data ----

my_data <- 
  my_data[complete.cases(my_data[, c("dsct_total", "sst_total", "fhat_total",
                                     "prof_e_total", "prof_f_total",
                                     "lt_e_total",   "lt_f_total",
                                     "ca_c_e_total", "ca_c_f_total",
                                     "sm_e_total",   "sm_f_total",
                                     "p_e_total",    "p_f_total",
                                     "mss_b_total",  "asq_total")]), ]
# Record new N of participants
my_results[["tracking_exclusions"]][2,"N"] <- nrow(my_data)


# Second round of filtering of bad entries (mainly aimed at participants not taking it seriously) ----

# Filter by completion time
my_data <- my_data[(my_data$duration_in_seconds >= 1800 | 
                      is.na(my_data$duration_in_seconds)), ]

# Filtering those who used a "cheat" strategy on DSCT ("strafing" across keys)
my_data <- my_data[(my_data$dsct_incorr < 50 | 
                      is.na(my_data$dsct_incorr)), ]

# Remove participants highlighted as not taking seriously in manual review
# See "AH_Thesis_screening_cols_EN.xlsx", "AH_Thesis_screening_cols_FR.xlsx", and "AH_Thesis_2024_ForQC_Completed.xlsx"
misc_qc_exclusions <- c("R_1ia5xPM59M1iZia", # wrote that this is taking too long instead of response
                        "R_br9JfJ0BgAKJ441", # gender = chimpanzee
                        "R_BM8jSsUOiNgjp9D", # gibberish (random letters)
                        "R_6qwgBgLQ4D6okCF") # gender = robot
my_data <- my_data[!(my_data$response_id %in% misc_qc_exclusions), ]
rm(misc_qc_exclusions)
...
# Remove blocks with high rates of missingness or sameness
# Missingness
for (my_block in 1:nrow(block_col_nums))
{
  block_name <- block_col_nums[my_block, "block"]
  my_max     <- block_col_nums[my_block, "num_questions"]
  my_total   <- paste0(block_name, "_total")
  my_blanks  <- paste0("blanks_", block_name)
  
  for (my_row in 1:nrow(my_data))
  {
    if (my_data[my_row, my_blanks] > (my_max / 8))
      my_data[my_row, my_total] <- NA
  }
}
rm(my_block, block_name, my_max, my_total, my_blanks, my_row)
# Sameness
for (my_block in blocks_can_repeat)
{
  my_max <- block_col_nums[block_col_nums$block == my_block,
                           "num_questions"]
  my_total <- paste0(my_block, "_total")
  my_same  <- paste0("same_", my_block)
  
  for (my_row in 1:nrow(my_data))
  {
    if (!is.na(my_data[my_row, my_same]))
      if (my_data[my_row, my_same] > (my_max * (95/100)))
        my_data[my_row, my_total] <- NA
  }
}
rm(my_block, my_max, my_total, my_same, my_row)

# Repeat complete cases check after removing more missingness and sameness
my_data <- 
  my_data[complete.cases(my_data[, c("dsct_total", "sst_total", "fhat_total",
                                     "prof_e_total", "prof_f_total",
                                     "lt_e_total",   "lt_f_total",
...
# Add number of participants after QC
my_results[["tracking_exclusions"]][3,"N"] <- nrow(my_data)


# Excluding Ps with SZ/ASD, Ps with SZ/ASD relatives ----
...
# Record new N of participants
my_results[["tracking_exclusions"]][4,"N"] <- nrow(my_data)

# Filter by diagnoses in first-degree relatives
my_data <- my_data[(my_data$diag_rel_sz == FALSE | 
                      is.na(my_data$diag_rel_sz)), ]
my_data <- my_data[(my_data$diag_rel_asd == FALSE | 
                      is.na(my_data$diag_rel_asd)), ]
# Record new N of participants
my_results[["tracking_exclusions"]][5,"N"] <- nrow(my_data)
...
# Same for LexTALE subtotals and weighted scores
my_data$lt_l1_words <- convert_totals_to_l1_or_l2(
    my_data, 1, "lt_e_words",    "lt_f_words")
my_data$lt_l2_words <- convert_totals_to_l1_or_l2(
    my_data, 0, "lt_e_words",    "lt_f_words")
my_data$lt_l1_nonwords <- convert_totals_to_l1_or_l2(
    my_data, 1, "lt_e_nonwords", "lt_f_nonwords")
my_data$lt_l2_nonwords <- convert_totals_to_l1_or_l2(
    my_data, 0, "lt_e_nonwords", "lt_f_nonwords")
...
# And now for AoA data
my_data$aoa_speak_l1 <- convert_totals_to_l1_or_l2(
    my_data, 1, "en_aoa_speak", "fr_aoa_speak")
my_data$aoa_speak_l2 <- convert_totals_to_l1_or_l2(
    my_data, 0, "en_aoa_speak", "fr_aoa_speak")
my_data$aoa_speak_flu_l1 <- convert_totals_to_l1_or_l2(
    my_data, 1, "en_aoa_speak_flu", "fr_aoa_speak_flu")
my_data$aoa_speak_flu_l2 <- convert_totals_to_l1_or_l2(
    my_data, 0, "en_aoa_speak_flu", "fr_aoa_speak_flu")
my_data$aoa_read_l1 <- convert_totals_to_l1_or_l2(
    my_data, 1, "en_aoa_read", "fr_aoa_read")
my_data$aoa_read_l2 <- convert_totals_to_l1_or_l2(
    my_data, 0, "en_aoa_read", "fr_aoa_read")
my_data$aoa_read_flu_l1 <- convert_totals_to_l1_or_l2(
    my_data, 1, "en_aoa_read_flu", "fr_aoa_read_flu")
my_data$aoa_read_flu_l2 <- convert_totals_to_l1_or_l2(
    my_data, 0, "en_aoa_read_flu", "fr_aoa_read_flu")
...
# Calculate percent scores from block totals ----
...
#
# mss_b = schizotypy
# asq = autistic trait load
# lt = LexTale Task (lexis)
# ca_c = Camel and Cactus Task (semantics)
# sm = Syntactic Modification Task (syntax)
# p = Pragmatics Task (pragmatics)
# dsct = Digit-Symbol Coding Task (non-verbal processing speed)
# sst = Spatial Span Task (non-verbal working memory)
# fhat = Frith-Happe Animations Task (non-verbal theory of mind)
# dlq = Demographic and Linguistic Questionnaire (mostly from LEAP-Q)


# ====== Descriptives ====== ----

# Populate my_results list with descr. statistics ----
...
my_results[["diag_counts"]] <- as.data.frame(matrix(NA, nrow = 11, ncol = 2))
colnames(my_results[["diag_counts"]]) <- c("dx", "N")
my_results[["diag_counts"]]$dx <- 
  c("ASD", "ADHD", "SLI", "Dyslexia", "OtherLearningDisability", 
    "Epilepsy", "SZ", "MoodDisorder", "AnxietyDisorder",
    "VisionProblem", "HearingImpairment")
...
my_results[["diag_counts"]][4,"N"]  <- sum(my_data$diag_dysl == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][5,"N"]  <- sum(my_data$diag_ld   == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][6,"N"]  <- sum(my_data$diag_epil == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][7,"N"]  <- sum(my_data$diag_sz   == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][8,"N"]  <- sum(my_data$diag_mood == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][9,"N"]  <- sum(my_data$diag_anx  == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][10,"N"] <- sum(my_data$diag_vis  == TRUE,
                                           na.rm = TRUE)
my_results[["diag_counts"]][11,"N"] <- sum(my_data$diag_hear == TRUE,
                                           na.rm = TRUE)
...
# Large histogram gallery for LEAP-Q variables ----
...
# 8. Histogram for number of languages
hist_num_langs <- ggplot(
    data=my_data, aes(x=num_langs, na.rm = TRUE)) +
    geom_histogram(fill=color_neutral, color="gray60",
                   binwidth=1, boundary=1.5,
                   na.rm=TRUE) +
    scale_x_continuous(breaks=2:7) + 
    xlab("Number of Languages") + ylab(NULL) +
    theme_bw() +
    theme(plot.title=element_text(size=11, hjust=0.5),
          axis.title.x=element_text(size=9),
          axis.title.y=element_text(size=9),
...
hist_profic + hist_exposure + hist_reading_choice + 
  hist_speaking_choice + hist_country_yrs + hist_family_yrs + 
  hist_school_work_yrs + hist_num_langs +
  plot_layout(design=design, guides = "collect") &
    theme(legend.position = "right")
rm(design)


# Small histogram gallery for subclinical symptoms ----

# 1. Histogram for MSS-B
hist_mss_b <- ggplot(
    data=my_data, aes(x=mss_b_total, na.rm = TRUE)) +
    geom_histogram(fill=color_sz, color="gray60",
                   binwidth=2, boundary=0,
                   na.rm=TRUE) +
    xlab("MSS-B Total Score") + ylab("Count") +
    ggtitle("Schizotypy") +
    theme_bw() +
    theme(plot.title=element_text(size=11, hjust=0.5),
          axis.title.x=element_text(size=9),
...
# Two subclinical symptom histograms together
hist_mss_b + hist_asq
...
violin_data <- my_data[,c("random_id",
...
violin_data$lt_l1_total   <- violin_data$lt_l1_total   * 100/30
violin_data$ca_c_l1_total <- violin_data$ca_c_l1_total * 100/16
violin_data$sm_l1_total   <- violin_data$sm_l1_total   * 100/16
violin_data$p_l1_total    <- violin_data$p_l1_total    * 100/18
violin_data$lt_l2_total   <- violin_data$lt_l2_total   * 100/30
violin_data$ca_c_l2_total <- violin_data$ca_c_l2_total * 100/16
violin_data$sm_l2_total   <- violin_data$sm_l2_total   * 100/16
violin_data$p_l2_total    <- violin_data$p_l2_total    * 100/18
...
ggplot(violin_data, aes(x = Task, y = score,
       ...
       scale_fill_manual(values = c(color_l1, color_l2),
                         name = "Language") +
       labs(x= NULL, y = "Mean Total Score (%)", title = NULL) +
       theme_minimal() +
       theme(strip.text = element_blank())


# ====== LME models for main analyses ====== ----

# Initialize the data frame ----

df_lmes <- my_data[,c("random_id",
                      "lt_l1_weighted", "lt_l2_weighted",
                      # "lt_l1_words", "lt_l2_words",
                      # "lt_l1_nonwords", "lt_l2_nonwords",
...
# This is a check
# Participants with missing data should be removed above
# This guarantees descr. and inf. statistics computed on same sample
if (any(is.na(df_lmes)))
{
  stop("df_lmes contains NA values.")
}
...
# df_lmes$lt_l1_words <- df_lmes$lt_l1_words   * 100/20
# df_lmes$lt_l2_words <- df_lmes$lt_l2_words   * 100/20
# df_lmes$lt_l1_nonwords <- df_lmes$lt_l1_nonwords * 100/10
# df_lmes$lt_l2_nonwords <- df_lmes$lt_l2_nonwords * 100/10
...
summary(inter_mss_b_1, adjust="fdr", by = NULL)
# Planned comparisons of simple interactions
pairs(inter_mss_b_1, adjust="fdr", by = NULL)
...
slopes_asq <- emtrends(mod_asq, ~ task,
                         ...
                         pbkrtest.limit = 8000,
                         lmerTest.limit = 8000)
test(slopes_asq, adjust="fdr")
# First-level contrasts (differences between tasks)
inter_asq <- contrast(slopes_asq, interaction = "pairwise")
summary(inter_asq, adjust="fdr", by=NULL)
...
mss_b_diff <- contrast(
  ...
  adjust = "none"        # no multiple comps adjustment for plotting
)
...
# Sensitivity analysis - subsampling ----

# Prep for subsampling
set.seed(42)   # To ensure reproducibility
participant_ids <- unique(df_lmes$random_id)
num_iter <- 1000  # Number of iterations per configuration
my_props <- seq(0.9, 0.1, -0.1)  # Create vector of proportions to test

# Create structure to contain subsampling results
subsamp_list <- vector("list", 3)
names(subsamp_list) <- c("raw", "combined", "results")
subsamp_list[["raw"]] <- vector("list", length(my_props))
subsamp_list[["combined"]] <- vector("list", length(my_props))
subsamp_list[["results"]] <- vector("list", length(my_props))

# Substructure and list item names
for (my_pos in 1:length(my_props))
{
  # Substructure - a list item for each iteration within raw
  subsamp_list[["raw"]][[my_pos]] <- vector("list", length(num_iter))
  # List item names
  names(subsamp_list[["raw"]])[my_pos] <- as.character(my_props[my_pos] * 100)
  names(subsamp_list[["combined"]])[my_pos] <- as.character(my_props[my_pos] * 100)
  names(subsamp_list[["results"]])[my_pos] <- as.character(my_props[my_pos] * 100)
}
rm(my_pos)

# Loop per subsample size
for (cur_pos in 1:length(my_props))
{
  for (cur_iter in 1:num_iter)
  {
    # Randomly select a subsample of participants
    cur_subsample <- 
      sample(participant_ids,
             size = round((my_props[cur_pos]) * (length(participant_ids))),
             replace = FALSE)
    
    # Create a df including only that subsample
    # This step is separate so that all rows are kept per participant
    cur_data <- df_lmes %>%
      filter(random_id %in% cur_subsample)
    
    # Create the LME model for this subsample
    cur_model <- lmer(score ~ task*lang*mss_b_total + 
                        prof_l1_l2_diff +
                        dsct_total + sst_total + fhat_total + 
                        (1 | random_id),
                      data = cur_data, REML = FALSE)
    
    # Obtain slopes
    cur_slopes <- emtrends(cur_model, ~ lang | task,
                           var = "mss_b_total",
                           pbkrtest.limit = nrow(cur_data),
                           lmerTest.limit = nrow(cur_data))
    
    # Obtain first-level contrasts (L1 - L2)
    cur_contrasts <- contrast(cur_slopes, interaction = "pairwise")
    subsamp_list[["raw"]][[cur_pos]][[cur_iter]] <- 
      summary(cur_contrasts)$estimate
  }
  
  # Create a df collating all the results for the current prop.
  subsamp_list[["combined"]][[cur_pos]] <- 
    as.data.frame(do.call(rbind, subsamp_list[["raw"]][[cur_pos]]))
  colnames(subsamp_list[["combined"]][[cur_pos]]) <- 
    c("C&C","LexTALE","Pragmatics","S.M.")
    
  # Create a df to contain the summary stats for the current prop.
  subsamp_list[["results"]][[cur_pos]] <- 
    as.data.frame(matrix(NA, nrow=7, ncol=4))
  colnames(subsamp_list[["results"]][[cur_pos]]) <- 
    c("C&C","LexTALE","Pragmatics","S.M.")
  rownames(subsamp_list[["results"]][[cur_pos]]) <- 
    c("mean","SD","median","CI_lower","CI_upper",
      "sign_stability_pos","sign_stability_neg")
  
  # Populate the summary stats df
  for (cur_task in 1:4)
  {
    # Extract vector once for clarity (optional but cleaner)
    x <- subsamp_list[["combined"]][[cur_pos]][,cur_task]
    
    # Mean
    subsamp_list[["results"]][[cur_pos]][1,cur_task] <- 
      mean(x)
    
    # SD
    subsamp_list[["results"]][[cur_pos]][2,cur_task] <- 
      sd(x)
    
    # Median
    subsamp_list[["results"]][[cur_pos]][3,cur_task] <- 
      median(x)
    
    # 95% CI lower bound
    subsamp_list[["results"]][[cur_pos]][4,cur_task] <- 
      quantile(x, probs = 0.025, na.rm = TRUE)
    
    # 95% CI upper bound
    subsamp_list[["results"]][[cur_pos]][5,cur_task] <- 
      quantile(x, probs = 0.975, na.rm = TRUE)
    
    # Proportion > 0 (sign stability, positive)
    subsamp_list[["results"]][[cur_pos]][6,cur_task] <- 
      mean(x > 0, na.rm = TRUE)
    
    # Proportion < 0 (sign stability, negative)
    subsamp_list[["results"]][[cur_pos]][7,cur_task] <- 
      mean(x < 0, na.rm = TRUE)
  }
  rm(x)
}
# rm(cur_pos, cur_iter, cur_subsample, cur_data,
#    cur_model, cur_slopes, cur_contrasts, cur_task)
